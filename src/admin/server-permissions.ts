...
import { requireAnyPermission, requirePermission } from "../server/auth/permissions";
...
export async function requireAdminServerAnyPermission(permissionCodes: string[]) {
  const config = getAuthConfig();
  const principal =
    config.mode === "database"
      ? await createAuthService().getSession(getCookie(config.sessionCookieName))
      : null;
  return requireAnyPermission(requireAuthenticatedUser(principal), permissionCodes);
}
